...
shinyServer(
  function(input, output) 
  {
    output$sumPlot <- renderPlot({
      ...
      plot(x, y, type="l", xlab="time or position", ylab="pressure or displacement", main="superposition");
    })
    output$componentsPlot <- renderPlot({
      ...
      f5 <- input$f5;
      f6 <- input$f6;
      plot(0,0, xlim=c(0,1), ylim=c(-1,1), type="n", xlab="time or position", ylab="pressure or displacement", main="harmonics");
